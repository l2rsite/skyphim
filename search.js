...
(function(win, doc) {
    ...
    var q2o = win.q2o,
        ...
        defaults = {
            ...
            chunk: 10,
            text: {
                title: 'Kết quả tìm kiếm <em>%s%</em>',
                loading: 'Đang tìm kiếm &hellip;',
                previous: 'Previous',
                next: 'Next',
                empty: 'Không tìm thấy phim <em>%s%</em>.',
                end: 'Không tìm thấy phim <em>%s%</em>.'
            },
            query: {
                'alt': 'json',
                'orderby': 'updated'
    ...
    function fit(e) {
      ...
      container.style.cssText = 'padding:0px !important; margin-top:50px;background-color:' + get_css('background-color') + ';color:' + get_css('color') + ';position:fixed;z-index:9999;top:' + (T + H) + 'px;left:' + L + 'px;width:' + W + 'px;max-height:' + (win.innerHeight - T - H) + 'px;overflow:auto;';
    ...
    win['_' + fn] = function($) {
        ...
        list = function(current) {
            ...
            if (size) {
              ...
              str += '<li style="align:center;width:100%;padding-left:20px;padding-right:20px;"><div "class="' + name + '-image ' + (has_image ? 'loading' : 'no-image') + '">';
                str += has_image ? '<table><tr><td><a href="' + url + '"' + (target ? ' target="' + target + '"' : "") + '><img alt="" src="' + current.media$thumbnail.url.replace(/\/s\d+(\-c)?\//g, '/' + size + '/') + '" style="width:50px;height:50px;">' : '<span class="img" style="width:50px;height:50px;"></a></td>';
             str += '<td style="padding-left:10px;><h5 class="' + name + '-title"><a href="' + url + '"' + (target ? ' target="' + target + '"' : "") + '>' + mark(current.title.$t) + '</a></h5></td></tr></table></li><hr/>';   str += '</div>';
            }
    ...
    function search_cache(q, i) {
        ...
        if (c[0]) {
            ...
            k.innerHTML = '<div class="' + name + '-results-' + p + '">' + text[p].replace('%s%', ent(query)) + '</div>';
